...
public class ConcentrationBoard {
    ...
    public class CardMatch {
        ...
        /**
         * Create a new instance from the two revealed cards and whether they match.
         *
    ...
    public ConcentrationBoard(int DIM, boolean cheat) throws ConcentrationException {
        ...
        for (int row=0; row<DIM; ++row) {
            for (int col=0; col<DIM; ++col) {
                this.board[row][col] = new ConcentrationCard(row, col, chars.removeFirst());
            }
        }
...
//    }
    ...
    public CardMatch reveal(int row, int col) throws ConcentrationException {
        if (row < 0 || row >= this.DIM || col < 0 || col >= this.DIM || !board[row][col].isHidden()) {
            throw new ConcentrationException("Invalid coordinates");
        } else {
            //If revealed card is null, store as revealed card. Otherwise, create CardMatch object with this card and revealed card
            if (this.revealedCard == null) {
                this.revealedCard = this.board[row][col];
                this.revealedCard.reveal();
            } else {
                ConcentrationCard card1 = revealedCard;
                ConcentrationCard card2 = board[row][col];
                card1.reveal();
                card2.reveal();
                if (card2.getLetter() == revealedCard.getLetter()) {
                    revealedCard = null;
                    matches++;
    ...
    public boolean gameOver() {
        return this.matches == (this.DIM * DIM) / 2;
    }
