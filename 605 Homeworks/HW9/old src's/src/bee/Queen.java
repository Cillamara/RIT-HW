...
public class Queen extends Bee {
    ...
    public void run() {
        while (this.beeHive.isActive()) {
            ...
            if (this.queensChamber.hasDrone() && this.beeHive.hasResources()) {
                this.beeHive.claimResources();
                System.out.println("Queen summons drone");
                this.queensChamber.summonDrone();
                System.out.println("Queen mated!");
                try {
                    sleep(MATE_TIME_MS);
                ...
                int numNewBees = 0;
                for (int i = 0; i < RandomBee.nextInt(MIN_NEW_BEES, MAX_NEW_BEES); i++) {
                    numNewBees++;
        ...
        synchronized (queensChamber) {queensChamber.notifyAll();}
