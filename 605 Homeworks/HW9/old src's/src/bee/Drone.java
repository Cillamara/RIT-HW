...
public class Drone extends Bee {
     ...
     */
    public void run() {
        synchronized (queensChamber.lock) {
            if (beeHive.isActive()) {
                while (true) {
                    if (queensChamber.getBeginningQueue().peek() == this) {
                        queensChamber.enterChamber(this);
                        queensChamber.getBeginningQueue().poll();
                        break;
                    }
                }
                while (!getMated() && !getRemoved()) {
                    try {
                        queensChamber.lock.wait();
