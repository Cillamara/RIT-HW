...
public class BeeHive {
    ...
    public BeeHive(int numDrones, int numNectarWorkers, int numPollenWorkers) {
        ...
        for (int i=0; i<numDrones; ++i ) {
            Bee newDrone = Bee.createBee(Role.DRONE, Resource.NONE, this);
            this.bees.add(newDrone);
            this.queensChamber.enterQueue.add((Drone) newDrone);
        }
        //Create the bees
     ...
     * bees trigger off this to stop doing their normal job and shutdown.
    ...
    public synchronized void addBee(Bee bee) {
        ...
        if (this.isActive()) {
            if (bee.getRole() == Role.DRONE) {
                this.queensChamber.enterQueue.add((Drone) bee);
            }
            bee.start();
        }
