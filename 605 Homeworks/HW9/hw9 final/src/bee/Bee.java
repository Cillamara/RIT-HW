...
public abstract class Bee extends Thread {
    ...
    protected final BeeHive beeHive;
    ...
    public static Bee createBee(Role role, Worker.Resource resource, BeeHive beeHive) {
        if (role == Role.QUEEN) {
            Queen newQueen = new Queen(beeHive);
            System.out.println("*BH* " + newQueen + " is born");
            return newQueen;
        } else if (role == Role.DRONE) {
            Drone newDrone = new Drone(beeHive);
            System.out.println("*BH* " + newDrone + " is born");
            return newDrone;
        } else if (role == Role.WORKER) {
            Worker newWorker = new Worker(resource, beeHive);
            System.out.println("*BH* " + newWorker + " is born");
            return newWorker;
        } else {
