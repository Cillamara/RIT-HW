...
public class PolyRoot {
    ...
    private static double newtonsMethod(int[] poly, double x0, int iter) {
        ...
        //Value of f(x0)
        double polyAtX = PolyEval.evaluate(poly, x0);
        //Value of f'(x0)
        double derivAtX = PolyEval.evaluate(PolyDerive.computeDerivative(poly), x0);
        //Value of x1 given x1 = x0 - f(x0)/f'(x0)
        double nextGuess = x0 - (polyAtX / derivAtX);
        //True if within error bounds
        boolean withinError = Math.abs(nextGuess - polyAtX) <= EPSILON;
        //If current guess is within 0 +- EPSILON, then return the root
        //Otherwise compute the next guess using the previous guess
        if (withinError || iter > MAX_ITERATIONS) {
            return x0;
        }else{
            return newtonsMethod(poly, nextGuess, iter+1);
        }
    }
}
